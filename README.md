...
- 브라우저를 종료 하더라도 게임을 이어서 할 수 있는 세이브 기능을 넣고 싶었습니다. 세이브 데이터의 저장 위치에 대해서 고민을 했는데 DB에 저장하는 것과 Local Storage에 저장하는 두 가지 방법을 모색했습니다. 로그인이 필요 없이 익명으로 이름을 작성하여 플레이하는 게임인 만큼 유저의 데이터를 따로 저장하지 않기 때문에 Local Storage에 세이브 데이터를 저장하는 방식을 선택했고, 이를 구현하기 위해 `redux-persist`를 사용하였습니다.
...
- Closure로 인해 useSelector로 가져온 `redux state`가 갱신되지 않는 현상이 발생했습니다.
- 이미지 이벤트를 실행하는 함수를 canvas 관련 util에 callback으로 넘겨줬는데, 해당 callback 안에 있는 Modal의 오픈 여부를 결정하는 조건인 `redux state`가 업데이트된 게 반영되지 않아서 비밀번호 관련 모달이 정상적으로 실행되지 않았습니다.
...
- 처음에는 이미지 로드와 canvas에 그리는 작업을 하나의 함수에서 실행했는데, 이를 해결하기 위해 각각 따로 분리하여 이미지 로드 함수를 promisify 하여 `new Image()` 로 선언한 이미지를 리턴해주면, `async/await`을 사용하여 해당 프로미스로 이미지를 받아 그것을 캔버스에 그려주는 방식으로 수정하였습니다.
...
- 처음 부트캠프를 시작할 때만 해도 프로젝트를 발표하는 날이 오긴 하는 걸까? 생각했지만 막상 시작하니 반 년에 가까운 기간이 찰나의 순간처럼 지나갔습니다. 아직도 취업에 대한 두려움과 내가 과연 잘 할 수 있을까, 라는 두려움이 있지만 바닐라코딩에서 보냈던 시간들을 바탕으로 잘 이겨낼 수 있을 것 같습니다. 💪
- 그동안 많은 과제들과 프로젝트를 거치면서 서로에게 힘이 되어준 동기들과 어려운 벽을 마주할 때마다 스스로 문제를 해결하는 방법을 알려주신 켄 님과 멘토님들, 그리고 바코 분들께 진심으로 감사합니다. 🙇‍♂️
